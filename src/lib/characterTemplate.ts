...
type Attribute = "STRENGTH" | "AGILITY" | "VIGOR" | "INTELLECT" | "PRESENCE";
...
export async function createCharacterTemplate(): Promise<any> {
  ...
  let counterAttackId,
    dodgeId,
    blockId: string = "";
  const allActionPresets = [
    ...createAllAttributeTests(characterId),
    createCounterAttack(characterId),
    createDodge(characterId),
    createBlock(characterId),
  ];
